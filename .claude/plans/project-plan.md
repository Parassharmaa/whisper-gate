...
- **Files:** `src/pulse_whisper/eval/gapped_eval.py`, `hallucination_rate.py`, `metrics.py`
- **Blocked by:** —
...
- **Files:** `src/pulse_whisper/training/` (new), config dataclasses
- **Blocked by:** —
...
- **Files:** `src/pulse_whisper/analysis/alpha_analysis.py`, `frequency_analysis.py`, `stats.py`
- **Blocked by:** —
- α growth tracking, learned ω frequency analysis
- Paired t-tests, Cohen's d, bootstrap CIs, win rates
- Visualization: degradation curves, ablation heatmaps, frequency spectra

## Phase 1: Experiments (Run)

### Task 6: Phase 1.1 — Baseline gapped evaluation (Whisper-Tiny)
- **Script:** `scripts/run_prototype.py`
- **Blocked by:** Tasks 3, 4
- Frozen Whisper-Tiny on test-clean with gaps 0/5/15/30/multi
- Collect WER + hallucination rate at each level
- Establish baseline degradation curve
- ~30 min, no training
...
- Re-run gapped eval, compare against baseline
- Look for ANY signal (WER or hallucination reduction)
- ~30 min
